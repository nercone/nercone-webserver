...
├── databases
│   ├── .gitkeep
│   ├── access_counter.txt - アクセスカウンターの数値を記録するテキストファイル
│   └── google_fonts.json - Google FontsCSSの24時間インメモリキャッシュファイル
├── logs
│   ├── .gitkeep
...
│       ├── logger.py - アクセスログを生成しPostgreSQLに書き込むロガー
│       ├── database.py - PostgreSQLアクセスログDBおよびファイルベースのアクセスカウンターを管理するモジュール
...
| `asyncpg` | PostgreSQL非同期クライアント (アクセスログDB) |

## リクエスト処理の流れ

```
クライアント -> Nginx (80/tcp, 443/tcp, 443/quic+udp) -> Uvicorn (8080/tcp)
    -> Middleware.__call__()
        ...
        -> logger.finalize_log() でアクセスログをPostgreSQLに書き込み
```
...
- **`Options`** - PostgreSQLの接続URL(`database_url`、環境変数`DATABASE_URL`から取得)・Scourの最小化オプションを定義する。
...
| `GET /echo` | アクセスログのJSON内容をそのまま返す。(デバッグ用) |
| `GET /status` | サーバー・コンテンツのバージョン・日替わり迷言・アクセスカウンターをJSONで返す。 |
| `GET /welcome` | AAアートのウェルカムメッセージをプレーンテキストで返す。 |
| `GET /assets/css/google-fonts.css` | Google FontsのCSSをプロキシし、24時間ファイルキャッシュ(`databases/google_fonts.json`)する。 |
...
**サムネイル生成:**
`assets/images/thumbnail/template/{template}.svg`のSVGテンプレート内の`__PATH__`・`__TITLE__`・`__DESCRIPTION__`プレースホルダーを置換した後、`resvg_py`で1200×630pxのPNGにラスタライズする。フォントとして`MesloBIZUD-Regular.ttf`・`InterBIZUD-Regular.ttf`・`InterBIZUD-Bold.ttf`を使用する。
...
CSS・JS・フォント・画像・動画には`Cache-Control: public, max-age=604800`ヘッダーが、それ以外には`Cache-Control: no-cache`が付与される(既存のヘッダーがある場合は上書きしない)。

### `logger.py`
アクセスログを収集しPostgreSQLに書き込む。

`log_access()`はリクエスト開始時に呼ばれ、ID・タイムスタンプ・送信元IP・ポート・信頼フラグ・スキーム・ホスト・ポート・メソッド・パス・リクエストヘッダーを含む`dict`を生成して返す。

`finalize_log()`はレスポンス送信後に呼ばれ、ステータスコード・所要時間(ms)・タイミング情報を追記してから`database.insert_access_log()`でPostgreSQLの`access_logs`テーブルに書き込む。
...
**PostgreSQL関連:** `asyncpg`を使用した接続プール管理と`access_logs`テーブルへのログ書き込みを担う。`init_db()`でプールを初期化しテーブルを作成、`close_pool()`でプールを閉じる(共にアプリのlifespan管理)。`insert_access_log()`で1リクエスト分のログを挿入する。
...
- アクセスログはPostgreSQLの`access_logs`テーブルに格納されます。接続先は環境変数`DATABASE_URL`で指定します(デフォルト: `postgresql://website:website@localhost:5432/website`)。
