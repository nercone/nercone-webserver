...
async def fake_error_page(request: Request):
    return render("/error/nginx", templates=templates, access_counter=None, request=request, headers={"Content-Security-Policy": "default-src 'self' 'unsafe-inline'; font-src 'self' fonts.gstatic.com; base-uri 'self'; form-action 'self'; upgrade-insecure-requests;"})
